...
public class QuestionRepositoryTest {
    ...
    @Test
    void test_findAllByIsAdminsFalse_userQuestionsReturned() {
        //ARRANGE
        User someUser = someUser();
        ...
        Question someQuestion = someQuestion(otherUser);
        Question otherQuestion = someQuestion(thirdUser);
        someQuestion.setIsAdmins(false);
        otherQuestion.setIsAdmins(false);
        Question adminsQuestion = someQuestion(true);
        Pageable page = PageRequest.of(0, 20);
        //ACT
        Page<Question> questions = questionRepository.findAllByIsAdminsFalse(someUser.getId(), page);
        //ASSERT
        assertThat(questions.getTotalElements()).isGreaterThan(0);
        assertThat(questions.getContent()).contains(someQuestion);
        assertThat(questions.getContent()).doesNotContain(adminsQuestion);
        assertThat(questions.getContent()).doesNotContain(otherQuestion);
    }

    @Test
    void test_findAllByIsAdminsFalse_communityQuestionsReturned() {
        //ARRANGE
        User someUser = someUser();
        Community someCommunity = someCommunity();
        Community otherCommunity = someCommunity();
        List<User> followers = new ArrayList<>();
        followers.add(someUser);
        someCommunity.setFollowers(followers);
        Question someQuestion = someQuestion(someCommunity);
        Question otherQuestion = someQuestion(otherCommunity);
        someQuestion.setIsAdmins(false);
        otherQuestion.setIsAdmins(false);
        Question adminsQuestion = someQuestion(true);
        Pageable page = PageRequest.of(0, 20);
    ...
    @Test
    void test_findAllByIsAdminsFalse_nothingReturned() {
        //ARRANGE
        User someUser = someUser();
        User otherUser = someUser("other@gmail.com");
        Question someQuestion = someQuestion(otherUser);
